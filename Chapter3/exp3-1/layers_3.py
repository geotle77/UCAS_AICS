...
class ContentLossLayer(object):
	...
	def forward(self, input_layer, content_layer):
		...
		## Begin
		loss = np.sum(np.square(input_layer - content_layer)) / (2 * np.prod(input_layer.shape))
		## End
		return loss
	def backward(self, input_layer, content_layer):
		# TODO： 计算内容损失的反向传播
		## Begin
		bottom_diff = (input_layer - content_layer) / (2 * np.prod(input_layer.shape))
		## End
		return bottom_diff
...
class StyleLossLayer(object):
	...
	def forward(self, input_layer, style_layer):
		...
		## Begin
		style_diff = np.sum(np.square(self.gram_input - self.gram_style)) / (self.div * 4 * input_layer.shape[0])
		loss = style_diff	# ? weight \omega is 1?
		## End
		return loss
	def backward(self, input_layer, style_layer):
		...
		for idxn in range(input_layer.shape[0]):
			# TODO： 计算风格损失的反向传播
			## Begin
			bottom_diff[idxn, :, :] = np.matmul((self.gram_input - self.gram_style)[idxn, :, :].T, self.input_layer_reshape[idxn, :, :]) / (input_layer.shape[0] * self.div)
			## End
