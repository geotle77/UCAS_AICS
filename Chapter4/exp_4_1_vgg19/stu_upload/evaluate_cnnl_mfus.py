...
if __name__ == '__main__':
    ...
    ## example_forward_input是一个随机生成的张量，
    ## 它的形状和模型的输入形状相同。这个张量的具体值并不重要，因为它只是用来让模型执行一次前向传播，
    ## 以便于torch.jit.trace函数可以跟踪模型的计算图。实际上，你可以使用任何具有正确形状的张量作为示例输入。
    
    #TODO: 使用JIT对模型进行trace，把动态图转化为静态图，得到net_trace
    net_trace = torch.jit.trace(net, example_forward_input)##将动态计算图转化为静态计算图，从而提高模型的运行效率。
    #TODO: 将输入图像拷贝到MLU设备
